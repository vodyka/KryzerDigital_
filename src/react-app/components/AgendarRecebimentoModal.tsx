...
interface AgendarRecebimentoModalProps {
  open: boolean;
...
export default function AgendarRecebimentoModal({
  open,
  onOpenChange,
  onSaved,
}: AgendarRecebimentoModalProps) {
    ...
    try {
      ...
      if (banksRes.ok) {
        ...
        if (accounts.length === 0) {
          setError(
            "Você precisa cadastrar pelo menos uma conta bancária antes de agendar um recebimento."
          );
        }
      }
    } catch (err) {
  ...
  const removeCategoryLine = (id: string) => {
    if (categoryLines.length === 1) return; // Keep at least one line
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError(errorData.error || "Erro ao agendar recebimento");
      }
    } catch (err) {
      console.error("Error saving receivable:", err);
      setError("Erro ao agendar recebimento");
    } finally {
      setLoading(false);
    }
  };
            ...
            <div className="flex items-center gap-3">
              <div className="flex flex-col">
                ...
                <span className="text-xs text-muted-foreground">
                  Registre um recebimento futuro e organize seu fluxo de caixa
                </span>
              </div>
