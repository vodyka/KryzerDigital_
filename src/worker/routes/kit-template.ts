...
// GET /api/kit-template - Get user's own template
app.get("/", async (c) => {
  ...
  try {
    ...
    console.error("Error fetching user kit template:", error);
    return c.json({ error: "Failed to fetch template" }, 500);
  }
});

// GET /api/kit-template/system - Get system template (public route)
app.get("/system", async (c) => {
  ...
  try {
    ...
    console.error("Error fetching system kit template:", error);
    return c.json({ error: "Failed to fetch system template" }, 500);
  }
});

// POST /api/kit-template - Upload user's own template
app.post("/", async (c) => {
  const authHeader = c.req.header("Authorization");
  if (!authHeader?.startsWith("Bearer ")) {
    return c.json({ error: "Unauthorized" }, 401);
  ...
  try {
    ...
    const fileName = `user-templates/kit-user${userId}-${Date.now()}.xlsx`;
    ...
    await db
      ...
      .bind(`user_${userId}`, "kit", fileName, file.name, url)
      .run();
...
// DELETE /api/kit-template - Delete user's own template
app.delete("/", async (c) => {
  const authHeader = c.req.header("Authorization");
  if (!authHeader?.startsWith("Bearer ")) {
    return c.json({ error: "Unauthorized" }, 401);
